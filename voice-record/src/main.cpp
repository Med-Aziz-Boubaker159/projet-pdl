...
#define ADC_SERVER_URL "http:/192.168.1.134:5003/adc_samples"
#define I2S_SERVER_URL "http://192.168.1.134:5003/i2s_samples"

WiFiClient client;
IPAddress serverIP(192, 168, 1, 14); 
...
void sendData(uint8_t* bytes, size_t count) {
  digitalWrite(2, HIGH);  // Indicate data transmission
  // Send the data
  client.write(bytes, count);
  client.flush();
  digitalWrite(2, LOW);   // Indicate data transmission complete
}
